...
SongInfo currentSong = {"", "", "", 0, false, 0, 0};
...
void metadata_callback(uint8_t attr_id, const uint8_t *value) {
    Serial.printf("==> AVRC metadata callback: id 0x%x", attr_id);
    if (value != nullptr) {
        Serial.printf(", value: %s", (char*)value);
    }
    Serial.println();

    if (value == nullptr) {
        Serial.println("Warning: Received null metadata value");
        return;
    }
    ...
    if (attr_id == ESP_AVRC_MD_ATTR_TITLE) {
        ...
        if (newTitle != currentSong.title) {
            ...
            // Reset scroll state khi có title mới
            scrollOffset = 0;
            scrollPause = true;
            pauseStartTimeScroll = millis();
        }
    ...
    // Thông báo cho OLED task cập nhật display
...
void track_changed_callback(uint8_t *id) {
    Serial.println("Track changed event received");
    ...
    Serial.println("Track changed - Metadata requested");
}

void playback_status_callback(esp_avrc_playback_stat_t playback) {
    // Giả định: nếu playback == ESP_AVRC_PLAYBACK_PLAYING thì coi là đang phát
    bool isPlaying = (playback == ESP_AVRC_PLAYBACK_PLAYING);
    Serial.printf("Playback status changed: %s (status code: %d)\n",
                  isPlaying ? "Playing" : "Paused", playback);
    portENTER_CRITICAL(&songInfoMux);
    currentSong.isPlaying = isPlaying;
    portEXIT_CRITICAL(&songInfoMux);
}

void play_position_callback(uint32_t pos) {
    Serial.printf("Play position updated: %lu ms\n", pos);
    portENTER_CRITICAL(&songInfoMux);
    currentSong.currentPosition = pos;
    currentSong.lastPosUpdate = millis();
    portEXIT_CRITICAL(&songInfoMux);
}

void avrc_connection_state_callback(bool connected) {
    Serial.printf("AVRC connection state: %s\n", connected ? "Connected" : "Disconnected");
    
    if (connected) {
        // Nếu kết nối lại, request metadata mới
        esp_avrc_ct_send_metadata_cmd(APP_RC_CT_TL_GET_META_DATA,
            ESP_AVRC_MD_ATTR_TITLE |
            ESP_AVRC_MD_ATTR_ARTIST |
            ESP_AVRC_MD_ATTR_PLAYING_TIME
        ...
        // Nếu mất kết nối AVRC, reset metadata
        portENTER_CRITICAL(&songInfoMux);
        currentSong = {"", "", "", 0, false, 0, 0};
        portEXIT_CRITICAL(&songInfoMux);
    }
...
bool isMetadataAvailable() {
    ...
    if (!a2dp_sink.is_avrc_connected()) {
        Serial.println("==> AVRC not connected");
        return false;
    }
    return true;
}
...
void setup() {
    ...
    // 1. Set metadata mask trước khi đăng ký callbacks
    ...
    // Đăng ký callback cho trạng thái kết nối AVRC
    a2dp_sink.set_avrc_connection_state_callback(avrc_connection_state_callback);
    
    // 3. Khởi tạo OLED Task
    startOLEDTask();
    
    // 4. Cuối cùng, start Bluetooth
    a2dp_sink.start("ESP32", true);
